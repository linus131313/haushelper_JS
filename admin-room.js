    ...
    import { getFirestore, collection, doc, setDoc, getDocs, getDoc, limit, query, updateDoc } from 'https://www.gstatic.com/firebasejs/9.4.1/firebase-firestore.js';
...
const formattedModifiedDate = new Date(firstFileModifiedTimestamp).toLocaleString('de-DE');
return { fileName: firstFileName, modifiedDate: formattedModifiedDate };
    } else {
      return null;
    }
  } catch (error) {
    console.error('Fehler Dateinamens:', error);
    throw error;
  }
}
...
document.getElementById("vornameProfil").innerHTML = docx.data().surname;
 document.getElementById("nachnameProfil").innerHTML = docx.data().name;
...
button.textContent = docz.data().address + ", " + docz.data().zipcode + " " + docz.data().city+"  ";
const filePath= docx.data().company + "/" + docz.data().address+", ("+docz.data().zipcode+")/Calendar";
...
defaultOption.text=existingData.employeeCalendar[address];}}}).catch((error) => {});
...
var selectedOption = userInput.options[userInput.selectedIndex].value;
getDocs(accessCollection)
.then((querySnapshot) => {
const firstDocument = querySnapshot.docs[0];
const existingData = firstDocument.data();
...
updateDoc(firstDocument.ref, newData).then(firstDocument => {}).catch(error => {
console.log(error);
});
 } else {
console.log('keine Dok');}}).catch((error) => {console.log('Dok:', error);
 });
 });
button.appendChild(userInput);
buttonContainer.appendChild(button);
const listItem = document.createElement('li');
listItem.textContent = docz.data().address + ", " + docz.data().zipcode + " " + docz.data().city;
...
document.getElementById("mitarbeiterX").innerHTML = docy.data().userAvailable;
document.getElementById("mitarbeiterY").innerHTML = docy.data().userInit;
 document.getElementById("gebaudeX").innerHTML = docy.data().facilityAvailable;
 document.getElementById("gebaudeY").innerHTML = docy.data().facilityInit;
 const timestamp_purchased = docy.data().purchased;const date = new Date(timestamp_purchased.seconds * 1000);
 const dateString = date.toLocaleDateString(); document.getElementById("date").innerHTML = dateString; });}).catch((error) => {console.log(error); });getDocs(userCollections).then((querySnapshot) => {const userList = document.querySelector('#userList'); querySnapshot.forEach((docw) => {const listItem = document.createElement('li');listItem.textContent = docw.data().name; userList.appendChild(listItem);});}).catch((error) => {console.error("Fehler Abrufen Dokumente: ", error);}); } else {}
});
 }).catch((error) => {console.error("Fehler Dokumente: ", error);
 });
const uid = user.uid; privateElements.forEach(function (element) {
